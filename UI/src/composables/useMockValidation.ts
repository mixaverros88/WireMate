...
/**
 * Validate the Response Transformers parameter JSON. Returns an empty
 * string when the text is empty (= "no parameters") or parses to a
 * plain JSON object; returns a human-readable error message otherwise.
 *
 * The parent's form validation and the section's live-feedback pass
 * call the same function so the two surfaces never disagree.
 */
export function validateTransformerParameters(jsonText: string): string {
