  ...
  <div class="min-h-screen" :class="isDark ? 'bg-gray-900' : 'bg-gray-50'">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div v-else class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          <!--
            Three-row stub card layout:
              Row 1 — Method badge + response status + priority chip
                      + persistent chip on the left; icon-only action
                      buttons (WireMate Stub, Logs, Delete) on the
                      right. Wraps on narrow widths so the actions
                      stay reachable without horizontal scroll.
              Row 2 — Stub UUID, monospaced.
              Row 3 — Request URL, monospaced.
          -->
          ...
          <div class="flex items-center gap-2 mb-3 flex-wrap">
            ...
            <div class="ml-auto flex items-center gap-1">
              ...
              <button
                ...
                class="inline-flex items-center justify-center w-8 h-8 rounded-md transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                :class="isDark
                ...
                :aria-label="wiremateProjectId(stub) ? 'Open in WireMate mock editor' : 'WireMate Stub (unavailable)'"
              >
          ...
          <div class="mb-2">
            <code :class="isDark ? 'text-gray-300 bg-gray-900' : 'text-gray-700 bg-gray-100'"
              class="text-xs px-2 py-1 rounded font-mono break-all inline-block">
              {{ stub.id }}
            </code>
          </div>

          <!-- Row 3 — Request URL -->
          <p :class="isDark ? 'text-gray-300' : 'text-gray-700'" class="text-sm font-mono break-all mb-4">
          ...
          <!-- Stub Details — the stub UUID is already shown as the
               prominent Row 2 badge above, so this footer only carries the
               ancillary fields (currently just the createdDate) to avoid
               rendering the same id twice on the card. -->
          <div
            v-if="stub.createdDate"
            :class="isDark ? 'border-gray-700' : 'border-gray-200'"
            class="border-t pt-4 text-xs space-y-2">
