...
import type { MockResponse, CreateMockPayload } from '../types/mock'
...
// Build the backend `CreateMockPayload` from an existing mock row. The
// backend (POST /api/mocks) owns the WireMock push — we hand it the
// same request/response/metadata we already know about and let it
// re-install the stub on the admin port on our behalf. Undefined fields
// are pruned so we don't ship a payload full of nulls.
function buildCreateMockPayloadFromMock(mock: MockResponse): CreateMockPayload {
  const payload: CreateMockPayload = {
    name: mock.name,
    projectId: mock.projectId,
    request: mock.request,
    response: mock.response,
    persistent: mock.persistent,
  }
  if (mock.description) payload.description = mock.description
  if (typeof mock.priority === 'number') payload.priority = mock.priority
  if (mock.scenarioName) payload.scenarioName = mock.scenarioName
  if (mock.requiredScenarioState) payload.requiredScenarioState = mock.requiredScenarioState
  if (mock.newScenarioState) payload.newScenarioState = mock.newScenarioState
  if (mock.metadata) payload.metadata = mock.metadata
  if (mock.serveEventListeners && mock.serveEventListeners.length > 0) {
    payload.serveEventListeners = mock.serveEventListeners
  }
  if (mock.curl) payload.curl = mock.curl
  return payload
}
...
async function createStubForMock(mock: MockResponse) {
  ...
  try {
    ...
    const payload = buildCreateMockPayloadFromMock(mock)
    await mockApi.update(mock.id, payload)
...
async function confirmDeleteMock() {
  ...
  try {
    ...
    error.value = 'Failed to delete mock'
  } finally {
...
async function confirmCloneMock(name: string) {
  ...
  try {
    ...
    error.value = e instanceof Error ? e.message : 'Failed to clone mock'
  } finally {
      ...
      <div v-else-if="project">
        <div class="rounded-lg border p-6 transition-colors"
          ...
          <div class="flex items-center gap-4 mb-6">
            ...
            <div>
              <h2 class="text-xl font-semibold" :class="isDark ? 'text-gray-100' : 'text-gray-800'">{{ project.name }}</h2>
