...
async function loadProjects() {
  ...
  try {
    ...
    showToastMessage('Failed to load projects', BaseToastEnum.ERROR)
  } finally {
    loading.value = false
  }
}
...
async function confirmClone(name: string) {
  ...
  try {
    ...
    if (cloned) {
      ...
      // Backend replied with an empty body (older server build, or a 204
      // No Content). The clone succeeded server-side, but we don't have
      // the new project in hand to append locally — re-fetch the list so
      // the UI still reflects the new row.
      await loadProjects()
    }
    showToastMessage('Project cloned successfully', BaseToastEnum.SUCCESS)
  } catch (e) {
