...
import { BaseButton, BaseButtonEnum, BaseCollapsibleSection, BaseSpinner, BaseToast, BaseToastEnum, useDebouncedRef, useTheme, useThemeClasses, useToast } from 'mgv-backoffice'
...
import ResponseTransformersSection from './mock-form/ResponseTransformersSection.vue'
...
// path-only — `urlPath` (Exact Path) or `urlPathPattern` (Path Regex).
// In both modes the user is opting into a path-only match and
// explicitly ignoring the query string, so allowing separate Query
// Parameter matchers would contradict the intent. We disable the
// controls (rather than hiding) so any previously-entered rows stay
// visible (and ignored at submit time, see buildRequest()).
const isQueryParamsDisabled = computed(() =>
  urlMatchType.value === 'urlPath' || urlMatchType.value === 'urlPathPattern'
)
...
const URL_MATCH_INFO: Record<string, string> = {
  url: 'Matches the full request URL exactly, including any query string. Use this when the query parameters are part of the identity of the request.',
  urlPath: 'Matches the request path exactly and ignores the query string. The Query Parameters section is disabled in this mode — switch to Exact URL or URL Regex if you need to assert on query parameters.',
  urlPattern: 'Matches the full URL (path + query string) against a Java regular expression. Use for pattern-based matching when the query string is part of the match.',
  urlPathPattern: 'Matches the request path against a Java regular expression and ignores the query string. Ideal for variable path segments like /users/{id}. The Query Parameters section is disabled in this mode — switch to URL Regex if you need to assert on query parameters.',
}
...
// request matching, body patterns, response transformer JSON, etc.)
// remain inline.
...
const validationIssues = computed<ValidationIssue[]>(() => {
  ...
  // current URL Match mode (urlPath / Exact Path) — those rows are
  // visually inert in the form and won't be submitted, so they
  // shouldn't trigger validation errors either.
  ...
  // WireMock and silently dropped from the wire payload. The same
  // pure validator drives the section's inline error.
  const transformerParamsError = validateTransformerParameters(responseTransformerParametersJson.value)
  if (transformerParamsError) {
    issues.push({ section: 'Response Transformers', message: transformerParamsError })
  }
...
function buildRequest(): RequestDefinition {
  ...
  // Query parameters — skipped entirely when URL Match is `urlPath`
  // (Exact Path). The form disables the controls in that mode so the
  // user can see any pre-existing rows, but we don't ship them in the
  // payload because the user has explicitly opted into a path-only
  // match that ignores the query string.
...
const curlCommand = computed(() => {
  ...
  // When URL Match is `urlPath`, the Query Parameters section is
  // disabled and intentionally ignored at submit time — mirror that
  // here so the preview matches what WireMock will actually receive.
    ...
    <form @submit.prevent="handleSubmit" class="space-y-6">
      ...
      >
        ...
        <!--
          ...
          urlMatchType is `urlPath`. We render an explanatory note so the
          user knows why the inputs are inert — picking "Exact Path"
          implicitly opts out of query-string matching.
        -->
        <div class="mb-6" :class="{ 'opacity-60': isQueryParamsDisabled }">
          <div class="flex items-center justify-between mb-2">
            ...
            <button type="button" @click="addMatcherRow(queryParams)"
              ...
              :title="isQueryParamsDisabled ? 'Query parameters are ignored when URL Match is set to “Exact Path”.' : ''"
          ...
          <p v-if="isQueryParamsDisabled"
            ...
            <span>Query Parameters are disabled because URL Match is set to <strong>{{ urlMatchType === 'urlPath' ? 'Exact Path' : 'Path Regex' }}</strong>, which matches the request path only and ignores the query string. Switch to <strong>Exact URL</strong> or <strong>URL Regex</strong> if you need to assert on query parameters.</span>
          </p>
      ...
      >
        ...
        <div class="mb-6">
          ...
          <div class="flex flex-wrap items-center gap-2 mb-2">
            ...
            <span v-if="uploadedFileName" class="text-xs" :class="t.mutedText">
      ...
      -->
      <ResponseTransformersSection
        :collapsed="collapsedSections.responseTransformers"
        :transformers="responseTransformers"
        :input="responseTransformerInput"
        :parameters-json="responseTransformerParametersJson"
        :fault-enabled="enableFault"
        @toggle="toggleSection('responseTransformers')"
        @update:transformers="responseTransformers = $event"
        @update:input="responseTransformerInput = $event"
        @update:parameters-json="responseTransformerParametersJson = $event"
      />
      ...
      >
          ...
          <div v-for="(cookie, i) in cookies" :key="i" class="mb-3 rounded-lg border p-3"
            ...
            <label v-if="cookie.direction === 'request' && supportsCaseInsensitive(cookie.matcherType)"
              class="flex items-center gap-1.5 text-xs mt-1 sm:ml-[calc(25%+0.5rem)] cursor-pointer"
              :class="labelClasses">
