...
<p align="center">
  ...
  Design, persist, and publish HTTP stubs from a clean UI — no hand-crafted JSON required.
</p>
...
- **One place for everything.** Mocks, active stubs, request journal, near misses, scenarios, and health all live under the same nav.
...
**Backend** — Java 25, Spring Boot 4, Spring Data JPA, PostgreSQL 17 (JSONB), Maven
...
```bash
git clone https://github.com/<your-org>/wiremate.git
cd wiremate

# Set GHCR_OWNER in .env to the lowercase GitHub org/user that owns the images,
# then either "docker login ghcr.io" (for private images) or ensure the
# packages are public in the GitHub package settings.

docker compose pull
docker compose up -d
```

This starts PostgreSQL on port 5432, WireMock on port 8080, the backend on port 8081, and the UI on port 3000 (served via Nginx). Pin a specific build by setting `IMAGE_TAG=main-<shortsha>` in `.env` before `docker compose up`.
...
| Method | Path                                | Description                          |
|--------|-------------------------------------|--------------------------------------|
| `POST` | `/api/wiremock/mocks/:id/publish`   | Manually publish a mock to WireMock  |
...
│   │   ├── controller/     # REST controllers (Project, Mock, WireMock, Backoffice)
│   │   ├── service/        # Domain services (Project, Mock, Notification)
...
WireMate builds and publishes JSON payloads like this to WireMock's admin API:
...
Each build also publishes immutable `main-<sha>` and `sha-<sha>` tags, so deployments can pin to an exact commit.
