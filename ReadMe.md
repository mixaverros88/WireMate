...
- **QA and integration testers** who need to fake upstream services on demand.
...
- **State Control** — Set global response delays, monitor server health, and perform admin operations from a single dashboard.
...
**Backend** — Java 25, Spring Boot 4.0.6, Spring Data JPA, PostgreSQL 17, Maven

**Frontend** — Vue 3.5, TypeScript 6.0, Vite 8, Tailwind CSS 4

**Infrastructure** — Docker & Docker Compose, Nginx, WireMock 3.6.0
...
**WireMate** at http://localhost:8081/

**WireMock** at http://localhost:8080/
...
- **Scenarios** — first-class state-machine support for stubs (a stub-level `scenarioName` + `requiredScenarioState` → `newScenarioState` triple, plus a Settings panel to view, reset, and step the active scenarios on WireMock). Removed from the UI (mock form section, project-detail badges, settings page panel) and from the supporting types/services. WireMock's underlying `/__admin/scenarios` API is still there if you need to drive it manually.
- **Traffic Recording** — the WireMate Settings page no longer exposes the Start / Stop / Snapshot controls that wrapped WireMock's `/__admin/recordings/*` endpoints. WireMock itself still supports recording; the UI affordance is what's pending.
