...
# GET /api/mocks/project/{projectId} — list mocks for project
echo "   GET /api/mocks/project/{projectId} (first project)"
curl -s -X GET "$BASE_URL/api/mocks/project/${PROJECT_IDS[0]}" > /dev/null
echo "      OK  ->  ${PROJECT_IDS[0]}"

# GET /api/backoffice/notifications — notifications
...
# =============================================================================
#  PHASE 5 — Publish mocks to WireMock  (POST /api/wiremock/mocks/{id}/publish)
# =============================================================================
echo ">> PHASE 5: Publishing first mock of each project to WireMock"
echo "   -----------------------------------------"
...
for (( p=0; p<N; p++ )); do
  ...
  curl -s -X POST "$BASE_URL/api/wiremock/mocks/$mid/publish" > /dev/null

  TOTAL_PUBLISHED=$((TOTAL_PUBLISHED + 1))
  echo "   [$TOTAL_PUBLISHED] Published mock $mid"
done
echo ""
...
echo "    Mocks published .............. $TOTAL_PUBLISHED"
