...
A Spring Boot REST API for managing WireMock stub mappings, organized by projects. 
WireMate lets you create, update, clone, and move mock API definitions and automatically publishes them to a running WireMock instance. 
A scheduled background task keeps the database and WireMock in sync and surfaces any drift as notifications consumable by the WireMate UI.
...
| POST   | `/api/mocks/projects/{projectId}/exists/by-name`      | 200      | Check whether a mock with the given name exists in project  |
| POST   | `/api/mocks/projects/{projectId}/exists/by-url`       | 200      | Check whether a mock with the given URL exists in project   |
...
| DELETE | `/api/backoffice/notifications/{notificationId}`  | 204    | Delete a single notification                             |
| DELETE | `/api/backoffice/notifications`                   | 204    | Delete all notifications                                 |

The notifications listing endpoint accepts the standard Spring `Pageable` query params (`page`, `size`, `sort`). Default page size is `100`, hard-capped at `500`. Default sort is `createdAt`.
